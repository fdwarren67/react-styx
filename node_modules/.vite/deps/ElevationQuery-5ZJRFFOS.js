...
var T = class {
  ...
  async _selectTilesAuto(t4, i) {
    ...
    const s2 = t4.getTilesToFetch(), a2 = {}, r = s2.map(async (t5) => {
      const n3 = new t(null, 0, 0, 0, u()), s3 = await _(l3.fetchAvailabilityUpsample(t5.level, t5.row, t5.col, n3, { signal: i }));
...
var w2 = class _w {
  ...
  async project(e2, t4) {
    ...
    const i = new u2({ spatialReference: this.spatialReference, points: this.coordinates.map((e3) => [e3.x, e3.y]) }), n3 = O(i, e2);
    if (!n3) return null;
  ...
  static fromGeometry(e2) {
    ...
    if (t4.geometry = e2, t4.spatialReference = e2.spatialReference, e2 instanceof _w) t4.coordinates = e2.coordinates.map((e3) => e3.clone()), t4._exporter = (t5, i) => {
      ...
      case "multipoint": {
        const i = e2, { hasZ: n3, hasM: o2 } = i;
        t4.coordinates = n3 && o2 ? i.points.map((e3) => new v(e3[0], e3[1], e3[2], e3[3])) : n3 ? i.points.map((e3) => new v(e3[0], e3[1], e3[2])) : o2 ? i.points.map((e3) => new v(e3[0], e3[1], null, e3[2])) : i.points.map((e3) => new v(e3[0], e3[1])), t4._exporter = (t5, i2) => e2.hasM ? new u2({ points: t5.map((e3) => [e3.x, e3.y, e3.z, e3.m]), hasZ: true, hasM: true, spatialReference: i2 }) : new u2(t5.map((e3) => [e3.x, e3.y, e3.z]), i2);
        break;
      }
      case "polyline": {
...
//# sourceMappingURL=ElevationQuery-5ZJRFFOS.js.map
