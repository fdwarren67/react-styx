...
var _C = class _C extends LitElement {
  ...
  async setFocus() {
    var _a, _b;
    await m3(this), !this.disabled && this.interactive ? (_a = this.containerEl.value) == null ? void 0 : _a.focus() : !this.disabled && this.closable && ((_b = this.closeButtonEl.value) == null ? void 0 : _b.focus());
  }
  async load() {
  ...
  renderSelectionIcon() {
    ...
    })}>${e ? html`<calcite-icon .icon=${e} .scale=${o(this.scale)}></calcite-icon>` : null}</div>`;
  }
  renderCloseButton() {
    return html`<button .ariaLabel=${this.messages.dismissLabel} class=${safeClassMap(c.close)} @click=${this.close} @keydown=${this.closeButtonKeyDownHandler} .tabIndex=${this.disabled ? -1 : 0} ${ref(this.closeButtonEl)}><calcite-icon .icon=${l.close} .scale=${o(this.scale)}></calcite-icon></button>`;
  }
  renderIcon() {
    return html`<calcite-icon class=${safeClassMap(c.chipIcon)} .flipRtl=${this.iconFlipRtl} .icon=${this.icon} .scale=${o(this.scale)}></calcite-icon>`;
  }
  render() {
    const { disabled: e } = this, s2 = e || !e && !this.interactive, p2 = this.selectionMode === "multiple" && this.interactive ? "checkbox" : this.selectionMode !== "none" && this.interactive ? "radio" : this.interactive ? "button" : "img";
    ...
    })} @click=${this.handleEmittingEvent} .role=${p2} .tabIndex=${s2 ? -1 : 0} ${ref(this.containerEl)}>${this.selectionMode !== "none" && this.renderSelectionIcon() || ""}${this.renderChipImage()}${this.icon && this.renderIcon() || ""}<span class=${safeClassMap(c.title)}><slot @slotchange=${this.handleDefaultSlotChange}></slot></span>${this.closable && this.renderCloseButton() || ""}</div>` });
  }
};
...
@esri/calcite-components/dist/components/calcite-chip/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=chunk-I5KOJPNS.js.map
