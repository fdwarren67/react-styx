import {
  s
} from "./chunk-FQILN67Z.js";
import {
  G,
  _,
  r
} from "./chunk-6K2FAOQG.js";

// node_modules/@arcgis/core/geometry/operators/gx/operatorGeodeticDensify.js
var o;
function s2() {
  return !!o && r();
}
async function i() {
  if (!s2()) {
    const [n, r2] = await Promise.all([import("./OperatorGeodeticDensifyByLength-XEPNGOXH.js"), import("./ProjectionTransformation-5RU2C5GV.js").then((n2) => n2.aG).then(({ injectPe: n2 }) => n2), _()]);
    o = new n.OperatorGeodeticDensifyByLength(), r2(G);
  }
}
function u(n, e, t, r2) {
  return o.execute(n, e, t, r2, null);
}
function c(e, t, r2, s3) {
  const i2 = o.executeMany(new s(e), t, r2, s3, null);
  return Array.from(i2);
}
function a() {
  return o.supportsCurves();
}

export {
  s2 as s,
  i,
  u,
  c,
  a
};
//# sourceMappingURL=chunk-QQKMDF7C.js.map
