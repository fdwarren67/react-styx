...
var _z = class _z extends LitElement {
  ...
  async setFocus() {
    var _a;
    await m2(this), (_a = this.layout === "vertical" ? this.el : this.headerEl.value) == null ? void 0 : _a.focus();
  }
  async load() {
  ...
  willUpdate(e) {
    e.has("selected") && (this.hasUpdated || this.selected !== false) && this.selectedHandler(), e.has("disabled") && (this.hasUpdated || this.disabled !== false) && this.registerStepperItem(), e.has("messages") && this.effectiveLocaleWatcher(this.messages._lang);
  }
  updated() {
    m(this), setAttribute(this.el, "tabindex", this.disabled || this.layout === "horizontal" ? null : 0);
  }
  // #endregion
  ...
  handleItemClick(e) {
    this.disabled || f(this.el) || this.layout === "horizontal" && e.composedPath().some((a) => {
      var _a;
      return (_a = a.classList) == null ? void 0 : _a.contains("stepper-item-content");
  ...
  render() {
    ...
    return p({ disabled: this.disabled, children: html`<div class=${safeClassMap(i.container)}>${this.complete && html`<span aria-live=polite class=${safeClassMap(i.visuallyHidden)}>${this.messages.complete}</span>` || ""}<div class=${safeClassMap(i.stepperItemHeader)} tabindex=${e ?? nothing} ${ref(this.headerEl)}>${this.icon ? this.renderIcon() : null}${this.numbered ? html`<div class=${safeClassMap(i.stepperItemNumber)}>${this.renderNumbers()}.</div>` : null}<div class=${safeClassMap(i.stepperItemHeaderText)}><span class=${safeClassMap(i.stepperItemHeading)}>${this.heading}</span><span class=${safeClassMap(i.stepperItemDescription)}>${this.description}</span></div></div><div class=${safeClassMap(i.stepperItemContent)}><slot></slot></div></div>` });
  }
  renderIcon() {
    let e = "circle";
    return this.selected && (this.layout !== "horizontal-single" || !this.error && !this.complete) ? e = "circleF" : this.error ? e = "exclamationMarkCircleF" : this.complete && (e = "checkCircleF"), html`<calcite-icon class="stepper-item-icon" .flipRtl=${this.iconFlipRtl} .icon=${e} scale=s></calcite-icon>`;
  }
  renderNumbers() {
    return q.numberFormatOptions = {
      ...
      numberingSystem: this.numberingSystem,
      useGrouping: false
    }, q.numberFormatter.format(this.itemPosition + 1);
  }
};
_z.properties = { complete: 7, description: 1, disabled: 7, error: 7, heading: 1, icon: 5, iconFlipRtl: 7, itemHidden: 7, layout: 3, messageOverrides: 0, numbered: 5, numberingSystem: 1, scale: 3, selected: 7 };
_z.styles = y;
var z = _z;
S("calcite-stepper-item", z);
export {
  z as StepperItem
};
...
@esri/calcite-components/dist/components/calcite-stepper-item/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-26LKDZ4C.js.map
