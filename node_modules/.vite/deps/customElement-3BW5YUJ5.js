...
var _M = class _M extends LitElement {
  ...
  // #region Lifecycle
  constructor() {
    super(), this.closeButtonEl = createRef(), this.cssVarObserver = b("mutation", () => {
    ...
    })(this), this.ignoreOpenChange = false, this.modalContent = createRef(), this.mutationObserver = b("mutation", () => this.focusTrap.updateContainerElements()), this._open = false, this.openEnd = () => {
      this.setFocus(), this.el.removeEventListener("calciteModalOpen", this.openEnd);
    }, this.openProp = "opened", this.transitionProp = "opacity", this.hasBack = false, this.hasContentBottom = false, this.hasContentTop = false, this.hasFooter = false, this.hasPrimary = false, this.hasSecondary = false, this.closeButtonDisabled = false, this.escapeDisabled = false, this.focusTrapDisabled = false, this.messages = s(), this.opened = false, this.outsideCloseDisabled = false, this.scale = "m", this.widthScale = "m", this.calciteModalBeforeClose = createEvent({ cancelable: false }), this.calciteModalBeforeOpen = createEvent({ cancelable: false }), this.calciteModalClose = createEvent({ cancelable: false }), this.calciteModalOpen = createEvent({ cancelable: false }), this.keyDownHandler = (e) => {
      const { defaultPrevented: i, key: c } = e;
      !i && this.focusTrapDisabled && this.open && !this.escapeDisabled && c === "Escape" && (e.preventDefault(), this.open = false);
  ...
  connectedCallback() {
    var _a, _b;
    super.connectedCallback(), (_a = this.mutationObserver) == null ? void 0 : _a.observe(this.el, { childList: true, subtree: true }), (_b = this.cssVarObserver) == null ? void 0 : _b.observe(this.el, { attributeFilter: ["style"] }), this.updateSizeCssVars();
  }
  async load() {
  ...
  willUpdate(e) {
    (e.has("hasBack") && (this.hasUpdated || this.hasBack !== false) || e.has("hasPrimary") && (this.hasUpdated || this.hasPrimary !== false) || e.has("hasSecondary") && (this.hasUpdated || this.hasSecondary !== false)) && (this.hasFooter = this.hasBack || this.hasPrimary || this.hasSecondary), e.has("opened") && (this.hasUpdated || this.opened !== false) && this.handleOpenedChange(this.opened);
  }
  disconnectedCallback() {
    var _a, _b;
    super.disconnectedCallback(), this.removeOverflowHiddenClass(), (_a = this.mutationObserver) == null ? void 0 : _a.disconnect(), (_b = this.cssVarObserver) == null ? void 0 : _b.disconnect();
  }
  handleHeaderSlotChange(e) {
    this.titleEl = ut(e)[0];
  }
  handleContentSlotChange(e) {
    this.contentEl = ut(e)[0];
  }
  handleBackSlotChange(e) {
    this.hasBack = ot(e);
  }
  handlePrimarySlotChange(e) {
    this.hasPrimary = ot(e);
  }
  handleSecondarySlotChange(e) {
    this.hasSecondary = ot(e);
  }
  setTransitionEl(e) {
  ...
  onBeforeOpen() {
    var _a;
    (_a = this.transitionEl) == null ? void 0 : _a.classList.add(t.openingActive), this.calciteModalBeforeOpen.emit();
  }
  onOpen() {
    var _a;
    (_a = this.transitionEl) == null ? void 0 : _a.classList.remove(t.openingIdle, t.openingActive), this.calciteModalOpen.emit(), this.focusTrap.activate();
  }
  onBeforeClose() {
    var _a;
    (_a = this.transitionEl) == null ? void 0 : _a.classList.add(t.closingActive), this.calciteModalBeforeClose.emit();
  }
  onClose() {
    var _a;
    (_a = this.transitionEl) == null ? void 0 : _a.classList.remove(t.closingIdle, t.closingActive), this.calciteModalClose.emit(), this.focusTrap.deactivate();
  }
  toggleModal(e) {
  ...
  handleOpenedChange(e) {
    ...
    i.classList.add(c), s2(this);
  }
  handleCloseClick() {
    this.open = false;
  }
  async openModal() {
    await this.componentOnReady(), this.el.addEventListener("calciteModalOpen", this.openEnd), this.opened = true, this.titleId = dt(this.titleEl), this.contentId = dt(this.contentEl), getNearestOverflowAncestor(this.el) === document.body && (r === 0 && (v = document.documentElement.style.overflow), r++, document.documentElement.style.setProperty("overflow", "hidden"));
  }
  handleOutsideClose() {
  ...
  async closeModal() {
    ...
    getNearestOverflowAncestor(this.el) === document.body && (r--, r === 0 && this.removeOverflowHiddenClass()), this.opened = false;
  }
  ...
  contentTopSlotChangeHandler(e) {
    this.hasContentTop = ot(e);
  }
  contentBottomSlotChangeHandler(e) {
    this.hasContentBottom = ot(e);
  }
  // #endregion
  // #region Rendering
  render() {
    return setAttribute(this.el, "aria-describedby", this.contentId), setAttribute(this.el, "aria-labelledby", this.titleId), this.el.ariaModal = "true", this.el.role = "dialog", html`<div class=${safeClassMap({
      [t.container]: true,
      [t.containerOpen]: this.opened
    })}><calcite-scrim class=${safeClassMap(t.scrim)} @click=${this.handleOutsideClose}></calcite-scrim>${this.renderStyle()}<div class=${safeClassMap({
      [t.modal]: true
    })} ${ref(this.setTransitionEl)}><div class=${safeClassMap(t.header)}>${this.renderCloseButton()}<header class=${safeClassMap(t.title)}><slot name=${t.header} @slotchange=${this.handleHeaderSlotChange}></slot></header></div>${this.renderContentTop()}<div class=${safeClassMap({
      [t.content]: true,
      [t.contentNoFooter]: !this.hasFooter
    })} ${ref(this.modalContent)}><slot name=${n.content} @slotchange=${this.handleContentSlotChange}></slot></div>${this.renderContentBottom()}${this.renderFooter()}</div></div>`;
  }
  renderFooter() {
    return keyed("footer", html`<div class=${safeClassMap(t.footer)} .hidden=${!this.hasFooter}><span class=${safeClassMap(t.back)}><slot name=${n.back} @slotchange=${this.handleBackSlotChange}></slot></span><span class=${safeClassMap(t.secondary)}><slot name=${n.secondary} @slotchange=${this.handleSecondarySlotChange}></slot></span><span class=${safeClassMap(t.primary)}><slot name=${n.primary} @slotchange=${this.handlePrimarySlotChange}></slot></span></div>`);
  }
  renderContentTop() {
    return html`<div class=${safeClassMap(t.contentTop)} .hidden=${!this.hasContentTop}><slot name=${n.contentTop} @slotchange=${this.contentTopSlotChangeHandler}></slot></div>`;
  }
  renderContentBottom() {
    return html`<div class=${safeClassMap(t.contentBottom)} .hidden=${!this.hasContentBottom}><slot name=${n.contentBottom} @slotchange=${this.contentBottomSlotChangeHandler}></slot></div>`;
  }
  renderCloseButton() {
    return this.closeButtonDisabled ? null : keyed("button", html`<button .ariaLabel=${this.messages.close} class=${safeClassMap(t.close)} @click=${this.handleCloseClick} title=${this.messages.close ?? nothing} ${ref(this.closeButtonEl)}><calcite-icon .icon=${B.close} .scale=${o(this.scale)}></calcite-icon></button>`);
  }
  renderStyle() {
    if (!this.fullscreen && (this.cssWidth || this.cssHeight))
      return html`<style>${`.${t.container} {
...
@esri/calcite-components/dist/components/calcite-modal/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-3BW5YUJ5.js.map
