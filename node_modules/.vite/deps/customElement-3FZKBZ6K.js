...
var _M = class _M extends LitElement {
  ...
  async setFocus() {
    var _a;
    await m2(this), !this.disabled && this.interactive && ((_a = this.containerEl) == null ? void 0 : _a.focus());
  }
  updated() {
    m(this);
  }
  ...
  handleSelectEvent() {
    this.disabled || !this.interactive || this.selectionMode === "single-persist" && this.selected === true || this.calciteTileSelect.emit();
  }
  handleSlotChange(e) {
  ...
  renderSelectionIcon() {
    ...
    if (c === "icon" && a !== "none")
      return html`<calcite-icon class=${safeClassMap(t.selectionIcon)} .icon=${e ? a === "multiple" ? s.selectedMultiple : s.selectedSingle : a === "multiple" ? s.unselectedMultiple : s.unselectedSingle} scale=s></calcite-icon>`;
  }
  renderTile() {
    const { description: e, disabled: c, hasContentBottom: a, hasContentEnd: g, hasContentStart: m3, hasContentTop: f, heading: l, icon: r, iconFlipRtl: x, interactive: i, selectionMode: h } = this, k = l && r && !e, v = !!this.href || !i, p2 = h === "multiple" && i ? "checkbox" : h !== "none" && i ? "radio" : i ? "button" : void 0, b = !!(e || g || m3 || l || r), z = !b && f && a;
    return html`<div .ariaChecked=${h !== "none" && i ? this.selected : void 0} .ariaDisabled=${v ? c : void 0} .ariaLabel=${p2 && this.label} class=${safeClassMap({
      [t.container]: true,
      [t.interactive]: i,
    ...
    })} @click=${this.clickHandler} .role=${p2} tabindex=${(v ? void 0 : 0) ?? nothing} ${ref(this.setContainerEl)}>${this.renderSelectionIcon()}<div class=${safeClassMap({
      [t.contentContainer]: true,
      [t.contentContainerHasContent]: b,
      [t.contentContainerHasOnlyContentTopAndBottom]: z
    })}><slot name=${d.contentTop} @slotchange=${this.handleSlotChange}></slot>${r && html`<calcite-icon class=${safeClassMap(t.icon)} .flipRtl=${x} .icon=${r} scale=l></calcite-icon>` || ""}<div class=${safeClassMap({ [t.textContentContainer]: true, [t.row]: true })}><slot name=${d.contentStart} @slotchange=${this.handleSlotChange}></slot><div class=${safeClassMap(t.textContent)}>${l && html`<div class=${safeClassMap(t.heading)}>${l}</div>` || ""}${e && html`<div class=${safeClassMap(t.description)}>${e}</div>` || ""}</div><slot name=${d.contentEnd} @slotchange=${this.handleSlotChange}></slot></div><slot name=${d.contentBottom} @slotchange=${this.handleSlotChange}></slot></div></div>`;
  }
  render() {
    const { disabled: e } = this;
    return p({ disabled: e, children: this.href ? html`<calcite-link .disabled=${e} .href=${this.href}>${this.renderTile()}</calcite-link>` : this.renderTile() });
  }
};
_M.properties = { hasContentBottom: 16, hasContentEnd: 16, hasContentStart: 16, hasContentTop: 16, active: 7, alignment: 3, description: 3, disabled: 7, embed: 7, heading: 3, href: 3, icon: 3, iconFlipRtl: 7, interactive: 5, label: 1, layout: 3, scale: 3, selected: 7, selectionAppearance: 3, selectionMode: 3 };
_M.styles = I;
var M = _M;
S("calcite-tile", M);
export {
...
@esri/calcite-components/dist/components/calcite-tile/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-3FZKBZ6K.js.map
