...
var I = class {
  ...
  unregisterElement(e) {
    ...
    i !== -1 && t.splice(i, 1), e.active = false, this.updateAlerts();
  }
  // --------------------------------------------------------------------------
  //
  //  Private Methods
...
var _H = class _H extends LitElement {
  ...
  // #region Lifecycle
  connectedCallback() {
    super.connectedCallback(), this.open && n2.registerElement(this.el), this.numberStringFormatter.numberFormatOptions = {
  ...
  willUpdate(e) {
    e.has("open") && (this.hasUpdated || this.open !== false) && this.openHandler(), e.has("active") && (this.hasUpdated || this.active !== false) && this.handleActiveChange(), e.has("autoCloseDuration") && (this.hasUpdated || this.autoCloseDuration !== "medium") && this.updateDuration(), e.has("queue") && (this.hasUpdated || this.queue !== "last") && this.handleQueueChange(), e.has("numberingSystem") && this.numberingSystemChange(), e.has("messages") && this.effectiveLocaleChange();
  }
  disconnectedCallback() {
    super.disconnectedCallback(), n2.unregisterElement(this.el), this.clearAutoCloseTimeout(), this.embedded = false;
  }
  // #endregion
  // #region Private Methods
  handleActiveChange() {
    s2(this), this.clearAutoCloseTimeout(), this.active && this.autoClose && !this.autoCloseTimeoutId && (this.initialOpenTime = Date.now(), this.autoCloseTimeoutId = window.setTimeout(() => this.closeAlert(), c[this.autoCloseDuration]));
  }
  openHandler() {
    this.open ? n2.registerElement(this.el) : n2.unregisterElement(this.el);
  }
  updateDuration() {
  ...
  handleQueueChange() {
    this.open && (n2.unregisterElement(this.el), n2.registerElement(this.el));
  }
  handleKeyBoardFocus() {
    this.isFocused = true, this.handleFocus();
  }
  handleKeyBoardBlur() {
    this.isFocused = false, this.isHovered || this.handleBlur();
  }
  ...
  closeAlert() {
    this.open = false, this.clearAutoCloseTimeout();
  }
  onBeforeOpen() {
  ...
  handleMouseOver() {
    this.isHovered = true, this.handleFocus();
  }
  handleMouseLeave() {
    this.isHovered = false, this.isFocused || this.handleBlur();
  }
  handleFocus() {
  ...
  // #region Rendering
  render() {
    const { open: e, autoClose: t, label: i, placement: g, active: d, openAlertCount: b } = this, v = t ? "alert" : "alertdialog", u = !e, h = Tt(n, this.icon, this.kind), C = b > 1;
    return this.el.inert = u, this.el.ariaLabel = i, this.el.toggleAttribute("calcite-hydrated-hidden", u), this.el.role = v, html`<div class=${safeClassMap({
      [s3.container]: true,
      [s3.containerActive]: d,
      [`${s3.container}--${g}`]: true,
      [s3.containerEmbedded]: this.embedded,
      [s3.focused]: this.isFocused
    })} @pointerenter=${this.autoClose && this.autoCloseTimeoutId ? this.handleMouseOver : null} @pointerleave=${this.autoClose ? this.handleMouseLeave : null} ${ref(this.setTransitionEl)}>${h && this.renderIcon(h) || ""}<div class=${safeClassMap(s3.textContainer)} @focusin=${this.autoClose && this.autoCloseTimeoutId ? this.handleKeyBoardFocus : null} @focusout=${this.autoClose ? this.handleKeyBoardBlur : null}><slot name=${r.title}></slot><slot name=${r.message}></slot><slot name=${r.link}></slot></div>${this.renderActionsEnd()}${C ? this.renderQueueCount() : null}${this.renderCloseButton()}${e && d && t ? html`<div class=${safeClassMap(s3.dismissProgress)}></div>` : null}</div>`;
  }
  renderCloseButton() {
    return keyed("close", html`<button .ariaLabel=${this.messages.close} class=${safeClassMap(s3.close)} @click=${this.closeAlert} @focusin=${this.autoClose ? this.handleKeyBoardFocus : null} @focusout=${this.autoClose ? this.handleKeyBoardBlur : null} type=button><calcite-icon icon=x .scale=${o(this.scale)}></calcite-icon></button>`);
  }
  renderQueueCount() {
    ...
    return keyed("queue-count", html`<div class=${safeClassMap({
      [s3.queueCount]: true,
      [s3.queueCountActive]: e > 1
    })}><calcite-chip .label=${i} .scale=${this.scale} .value=${i}>${i}</calcite-chip></div>`);
  }
  renderActionsEnd() {
    return html`<div class=${safeClassMap(s3.actionsEnd)}><slot name=${r.actionsEnd} @slotchange=${this.actionsEndSlotChangeHandler}></slot></div>`;
  }
  renderIcon(e) {
    return html`<div class=${safeClassMap(s3.icon)}><calcite-icon .flipRtl=${this.iconFlipRtl} .icon=${e} .scale=${o(this.scale)}></calcite-icon></div>`;
  }
};
_H.properties = { hasEndActions: 16, isFocused: 16, numberStringFormatter: 16, active: 5, autoClose: 7, autoCloseDuration: 3, embedded: 5, icon: [3, { converter: stringOrBoolean }], iconFlipRtl: 7, kind: 3, label: 1, messageOverrides: 0, numberingSystem: 3, open: 7, openAlertCount: 9, placement: 3, queue: 3, scale: 3 };
_H.styles = D;
var H = _H;
S("calcite-alert", H);
export {
...
@esri/calcite-components/dist/components/calcite-alert/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-CX67X4HP.js.map
