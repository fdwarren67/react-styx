...
var __ = class __ extends LitElement {
  ...
  handleKeyDown(e) {
    const { key: t, defaultPrevented: s3, shiftKey: n } = e, { position: i, resizable: a, contentEl: h, el: r2, resizeValues: { maxBlockSize: z, maxInlineSize: u, minBlockSize: x, minInlineSize: f } } = this, v = [...i === "block-end" || i === "block-start" ? ["ArrowUp", "ArrowDown"] : ["ArrowLeft", "ArrowRight"], "Home", "End"];
    if (!a || !h || s3 || !v.includes(t))
      return;
    const c = this.getContentElDOMRect(), b2 = gt(r2) === "rtl" ? -1 : 1, o = n ? F : U;
    switch (t) {
      case "ArrowUp":
        this.updateSize({
      ...
      case "ArrowLeft":
        this.updateSize({
          size: c.width + (i === "inline-end" ? o : -o) * b2,
          type: "inlineSize"
        }), e.preventDefault();
        break;
      case "ArrowRight":
  ...
  updateSize({ type: e, size: t }) {
    ...
    if (!s3)
      return;
    const i = e === "blockSize" ? "minBlockSize" : "minInlineSize", a = e === "blockSize" ? "maxBlockSize" : "maxInlineSize", h = n[i] && n[a] ? m2(t, n[i], n[a]) : t, r2 = Math.round(h);
    this.resizeValues = {
      ...n,
      [e]: r2
    }, s3.style[e] = t !== null ? `${r2}px` : null;
  }
  cleanupInteractions() {
    var _a;
  ...
  setupInteractions() {
    ...
    if (!t || !i || !s3 || !a)
      return;
    const { inlineSize: h, minInlineSize: r2, blockSize: z, minBlockSize: u, maxInlineSize: x, maxBlockSize: f } = window.getComputedStyle(t), d = {
      inlineSize: Ot(h) ? parseInt(h) : 0,
      blockSize: Ot(z) ? parseInt(z) : 0,
      minInlineSize: Ot(r2) ? parseInt(r2) : 0,
    ...
    this.resizeValues = d;
    const v = gt(e) === "rtl";
    this.interaction = Xr(t, { context: e.ownerDocument }).resizable({
      edges: {
  ...
  // #region Rendering
  render() {
    const { resizable: e, position: t, resizeValues: s3 } = this, n = gt(this.el), i = t === "block-start" || t === "block-end";
...
@esri/calcite-components/dist/components/calcite-sheet/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-NKPNKHM4.js.map
