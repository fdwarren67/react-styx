...
var _L = class _L extends LitElement {
  ...
  focusHandler(e) {
    const n = e.target;
    this.isFocused = true, n.open && !this.open && (n.open = false);
  }
  handleMenuItemSlotChange(e) {
    this.submenuItems = ut(e), this.submenuItems.forEach((n) => {
  ...
  async keyDownHandler(e) {
    const { hasSubmenu: n, href: c, layout: h, open: a, submenuItems: d } = this, i = e.key, s2 = e.target === this.dropdownActionEl.value;
    if (!e.defaultPrevented) {
      if (i === " " || i === "Enter")
        n && (!c || c && s2) && (this.open = !a), !(c && s2) && i !== "Enter" && this.selectMenuItem(e), (i === " " || c && s2) && e.preventDefault();
      else if (i === "Escape") {
        if (a) {
          this.open = false;
          return;
        ...
        if (e.preventDefault(), (s2 || !c) && n && !a && h === "horizontal") {
          this.open = true;
          return;
        }
        ...
        if (e.preventDefault(), (s2 || !c) && n && !a && h === "vertical") {
          this.open = true;
          return;
        }
        this.calciteInternalMenuItemKeyEvent.emit({
  ...
  // #region Rendering
  renderIconStart() {
    return keyed(t.iconStart, html`<calcite-icon class=${`${t.icon} ${t.iconStart}`} .flipRtl=${this.iconFlipRtl === "start" || this.iconFlipRtl === "both"} .icon=${this.iconStart} scale=s></calcite-icon>`);
  }
  renderIconEnd() {
    return keyed(t.iconEnd, html`<calcite-icon class=${`${t.icon} ${t.iconEnd}`} .flipRtl=${this.iconFlipRtl === "end" || this.iconFlipRtl === "both"} .icon=${this.iconEnd} scale=s></calcite-icon>`);
  }
  renderBreadcrumbIcon(e) {
    return keyed(t.iconBreadcrumb, html`<calcite-icon class=${`${t.icon} ${t.iconBreadcrumb}`} .icon=${e === "rtl" ? "chevron-left" : "chevron-right"} scale=s></calcite-icon>`);
  }
  renderDropdownIcon(e) {
    const n = e === "rtl" ? "chevron-left" : "chevron-right";
    return keyed(t.iconDropdown, html`<calcite-icon class=${`${t.icon} ${t.iconDropdown}`} .icon=${this.topLevelMenuLayout === "vertical" || this.isTopLevelItem ? this.open ? "chevron-up" : "chevron-down" : n} scale=s></calcite-icon>`);
  }
  renderDropdownAction(e) {
    const n = e === "rtl" ? "chevron-left" : "chevron-right";
    return keyed(t.dropdownAction, html`<calcite-action class=${safeClassMap(t.dropdownAction)} .icon=${this.topLevelMenuLayout === "vertical" || this.isTopLevelItem ? this.open ? "chevron-up" : "chevron-down" : n} @click=${this.clickHandler} @keydown=${this.keyDownHandler} .text=${this.messages.open} ${ref(this.dropdownActionEl)}></calcite-action>`);
  }
  renderSubmenuItems(e) {
    return html`<calcite-menu class=${safeClassMap({
      [t.dropdownMenuItems]: true,
      [t.open]: this.open,
  ...
  renderHrefIcon(e) {
    return keyed(t.hoverHrefIcon, html`<calcite-icon class=${safeClassMap(t.hoverHrefIcon)} .icon=${e === "rtl" ? "arrow-left" : "arrow-right"} scale=s></calcite-icon>`);
  }
  renderItemContent(e) {
    const n = this.href && (this.topLevelMenuLayout === "vertical" || !this.isTopLevelItem);
    return html`${this.iconStart && this.renderIconStart() || ""}<div class=${safeClassMap(t.textContainer)}><span>${this.text}</span></div>${n && this.renderHrefIcon(e) || ""}${this.iconEnd && this.renderIconEnd() || ""}${this.breadcrumb ? this.renderBreadcrumbIcon(e) : null}${!this.href && this.hasSubmenu ? this.renderDropdownIcon(e) : null}`;
  }
  render() {
    const e = gt(this.el);
    ...
    })} role=none><div class=${safeClassMap(t.itemContent)}><a .ariaCurrent=${this.isFocused ? "page" : false} .ariaExpanded=${this.open} .ariaHasPopup=${this.hasSubmenu} .ariaLabel=${this.label} class=${safeClassMap({ [t.layoutVertical]: this.layout === "vertical", [t.content]: true })} href=${this.href ?? nothing} @click=${this.clickHandler} @keydown=${this.keyDownHandler} rel=${this.rel ?? nothing} role=menuitem .tabIndex=${this.isTopLevelItem ? 0 : -1} target=${this.target ?? nothing} ${ref(this.anchorEl)}>${this.renderItemContent(e)}</a>${this.href && this.hasSubmenu ? this.renderDropdownAction(e) : null}</div>${this.renderSubmenuItems(e)}</li>`;
  }
};
_L.properties = { hasSubmenu: 16, submenuItems: 16, active: 7, breadcrumb: 7, href: 1, iconEnd: 3, iconFlipRtl: 3, iconStart: 3, isTopLevelItem: 5, label: 1, layout: 3, messageOverrides: 0, open: 7, rel: 3, target: 3, text: 1, topLevelMenuLayout: 1 };
_L.styles = k;
var L = _L;
S("calcite-menu-item", L);
export {
  L as MenuItem
};
...
@esri/calcite-components/dist/components/calcite-menu-item/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-OR2JSZQZ.js.map
