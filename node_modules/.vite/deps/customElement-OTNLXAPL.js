...
var _A = class _A extends LitElement {
  ...
  // #region Lifecycle
  constructor() {
    super(), this.componentToNativeEl = /* @__PURE__ */ new Map(), this.mutationObserver = b("mutation", () => this.populateInternalSelect()), this.disabled = false, this.required = false, this.scale = "m", this.status = "idle", this.validity = {
      valid: false,
      badInput: false,
      customError: false,
      ...
      typeMismatch: false,
      valueMissing: false
    }, this.value = null, this.width = "auto", this.calciteSelectChange = createEvent({ cancelable: false }), this.listen("calciteInternalOptionChange", this.handleOptionOrGroupChange), this.listen("calciteInternalOptionGroupChange", this.handleOptionOrGroupChange);
  }
  // #endregion
  // #region Public Methods
  /** Sets focus on the component. */
  async setFocus() {
    await m2(this), tt(this.selectEl);
  }
  connectedCallback() {
    var _a;
    super.connectedCallback();
    const { el: e } = this;
    (_a = this.mutationObserver) == null ? void 0 : _a.observe(e, {
      subtree: true,
      childList: true
    }), v(this), D(this);
  }
  willUpdate(e) {
    var _a;
    e.has("value") && (this.hasUpdated || this.value !== null) && this.updateItemsFromValue(this.value), e.has("selectedOption") && (this.value = (_a = this.selectedOption) == null ? void 0 : _a.value);
  }
  updated() {
    m(this);
  }
  loaded() {
    ...
    this.selectFromNativeOption(e), O(this, ((_a = this.selectedOption) == null ? void 0 : _a.value) ?? "");
  }
  disconnectedCallback() {
    var _a;
    super.disconnectedCallback(), (_a = this.mutationObserver) == null ? void 0 : _a.disconnect(), T(this), W(this);
  }
  // #endregion
  ...
  populateInternalSelect() {
    ...
    this.clearInternalSelect(), e.forEach((t) => {
      var _a;
      return (_a = this.selectEl) == null ? void 0 : _a.append(this.toNativeElement(t));
    });
  }
  clearInternalSelect() {
  ...
  selectFromNativeOption(e) {
    ...
    this.componentToNativeEl.forEach((i, a) => {
      s(a) && i === e && (a.selected = true, t = a, this.deselectAllExcept(a));
  ...
  // #region Rendering
  renderChevron() {
    return html`<div class=${safeClassMap(n.iconContainer)}><calcite-icon class=${safeClassMap(n.icon)} icon=chevron-down .scale=${o(this.scale)}></calcite-icon></div>`;
  }
  render() {
    const { disabled: e } = this;
    return p({ disabled: e, children: html`<div class=${safeClassMap(n.wrapper)}><select aria-errormessage=${r.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${I(this)} class=${safeClassMap(n.select)} .disabled=${e} @change=${this.handleInternalSelectChange} ${ref(this.storeSelectRef)}><slot></slot></select>${this.renderChevron()}${B({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? m3({ icon: this.validationIcon, id: r.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
};
_A.properties = { disabled: 7, form: 3, label: 1, name: 3, required: 7, scale: 3, selectedOption: 0, status: 3, validationIcon: [3, { converter: stringOrBoolean }], validationMessage: 1, validity: 0, value: 1, width: 3 };
_A.styles = T2;
var A = _A;
S("calcite-select", A);
export {
...
@esri/calcite-components/dist/components/calcite-select/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-OTNLXAPL.js.map
