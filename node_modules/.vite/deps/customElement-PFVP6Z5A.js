...
var _H = class _H extends LitElement {
  ...
  load() {
    this.parentDatePickerEl = m(this.el, "calcite-date-picker");
  }
  willUpdate(e) {
  ...
  setNextPrevMonthDates() {
    this.activeDate && (this.nextMonthDate = p2(N(this.activeDate), this.min, this.max), this.prevMonthDate = p2(A(this.activeDate), this.min, this.max));
  }
  /**
   ...
   */
  onYearKey(e) {
    const t2 = this.parseCalendarYear(e.target.value);
    switch (e.key) {
      case "ArrowDown":
        e.preventDefault(), this.setYear({ localizedYear: t2, offset: -1 });
        break;
      case "ArrowUp":
        e.preventDefault(), this.setYear({ localizedYear: t2, offset: 1 });
        break;
  ...
  formatCalendarYear(e) {
    return q.localize(`${I(e, this.localeData)}`);
  }
  parseCalendarYear(e) {
    return q.localize(`${M(Number(q.delocalize(e)), this.localeData)}`);
  }
  onYearChange(e) {
    this.setYear({
  ...
  prevMonthKeydown(e) {
    t(e.key) && this.prevMonthClick(e);
  }
  nextMonthClick(e) {
    this.handleArrowClick(e, this.nextMonthDate);
  }
  nextMonthKeydown(e) {
    t(e.key) && this.nextMonthClick(e);
  }
  async handleArrowClick(e, t2) {
    e.preventDefault(), await this.handlePenultimateValidMonth(e), this.calciteInternalDatePickerMonthHeaderSelectChange.emit(t2);
  }
  handleMonthChange(e) {
    const t2 = e.target, { abbreviated: a, wide: n } = this.localeData.months, i = (this.monthStyle === "wide" ? n : a).indexOf(t2.value);
    let l = C(this.activeDate, i);
    m2(l, this.min, this.max) || (l = p2(l, this.min, this.max)), this.calciteInternalDatePickerMonthHeaderSelectChange.emit(l), this.setYearSelectMenuWidth();
  }
  getInRangeDate({ localizedYear: e, offset: t2 = 0 }) {
    const { min: a, max: n, activeDate: c } = this, i = Number(q.delocalize(e)), l = i.toString().length, d = isNaN(i) ? false : i + t2, D = d && (!a || a.getFullYear() <= d) && (!n || n.getFullYear() >= d);
    if (d && D && l === e.length) {
      const v = new Date(c);
      return v.setFullYear(d), p2(v, a, n);
    }
  }
  /**
   ...
   * @param root0.offset
   */
  setYear({ localizedYear: e, commit: t2 = true, offset: a = 0 }) {
    const { yearInputEl: { value: n }, activeDate: c } = this, i = this.getInRangeDate({ localizedYear: e, offset: a });
    i && this.calciteInternalDatePickerMonthHeaderSelectChange.emit(i), t2 && (n.value = this.formatCalendarYear((i || c).getFullYear()));
  }
  ...
  setYearSelectMenuWidth() {
    ...
    e && requestAnimationFrame(() => {
      const t2 = getComputedStyle(e), a = `${t2.fontStyle} ${t2.fontVariant} ${t2.fontWeight} ${t2.fontSize}/${t2.lineHeight} ${t2.fontFamily}`, c = this.localeData.months[this.monthStyle][this.activeDate.getMonth()], i = Math.ceil(pt(c, a));
      e.style.width = `${i + this.yearSelectWidthOffset}px`;
    });
  }
  isMonthInRange(e) {
    const t2 = C(this.activeDate, e);
    return !this.min && !this.max || m2(t2, this.min, this.max) ? true : _(t2, this.max) || _(t2, this.min);
  }
  async handlePenultimateValidMonth(e) {
    const n = e.target.getAttribute("data-direction") === "left";
    let c;
    if (n && this.min) {
      const i = p2(A(this.activeDate), this.min, this.max);
      c = _(i, this.min);
    } else if (this.max) {
      const i = p2(N(this.activeDate), this.min, this.max);
      c = _(i, this.max);
    }
    if (c)
      if (this.position)
        ...
        i.disabled = false, await i.setFocus();
      }
  }
  getPx(e) {
    const t2 = Number(e.replace(/[rem|px]/g, "")), a = 16;
    return e.includes("rem") ? `${t2 * a}px` : `${t2}px`;
  }
  ...
  // #region Rendering
  render() {
    return html`<div class=${safeClassMap(s.header)}>${this.renderContent()}</div>`;
  }
  renderContent() {
    const { localeData: e, activeDate: t2 } = this;
    if (!t2 || !e)
      return null;
    ...
    const a = S2(e.unitOrder), n = a.indexOf("y") < a.indexOf("m");
    return html`${this.position && html`<div class=${safeClassMap({ [s.chevronContainer]: true })}>${this.position === "start" && this.renderChevron("left") || ""}</div>` || ""}<div class=${safeClassMap({
      [s.monthYearContainer]: true,
      [s.rangeCalendar]: !!this.position
    })}>${this.renderMonthYearContainer(n)}</div>${!this.position && html`<div class=${safeClassMap({ [s.chevronContainer]: true })}>${this.renderChevron("left")}</div>` || ""}<div class=${safeClassMap({ [s.chevronContainer]: true })}>${this.position !== "start" && this.renderChevron("right") || ""}</div>`;
  }
  ...
  renderMonthPicker() {
    const e = this.activeDate.getMonth(), t2 = this.localeData.months[this.monthStyle];
    return html`<calcite-select class=${safeClassMap(s.monthPicker)} .label=${this.messages.monthMenu} @calciteSelectChange=${this.handleMonthChange} width=auto ${ref(this.monthPickerEl)}>${t2.map((a, n) => html`<calcite-option .disabled=${!this.isMonthInRange(n)} .selected=${n === e} .value=${a}>${a}</calcite-option>`)}</calcite-select>`;
  }
  renderYearInput() {
    var _a;
    const e = (_a = this.localeData.year) == null ? void 0 : _a.suffix, t2 = this.formatCalendarYear(this.activeDate.getFullYear());
    return html`<span class=${safeClassMap(s.yearContainer)}><input .ariaLabel=${this.messages.year} class=${safeClassMap({
      year: true
    })} inputmode=numeric maxlength=4 minlength=1 @change=${this.onYearChange} @input=${this.onYearInput} @keydown=${this.onYearKey} pattern=\\d* type=text .value=${live(t2 ?? "")} ${ref(this.yearInputEl)}>${e && html`<span class=${safeClassMap(s.suffix)}>${e}</span>` || ""}</span>`;
  }
  renderChevron(e) {
    const t2 = e === "right", a = _(t2 ? this.nextMonthDate : this.prevMonthDate, this.activeDate) || !m2(this.activeDate, this.min, this.max);
    return html`<calcite-action alignment=center .ariaDisabled=${a} .ariaLabel=${t2 ? this.messages.nextMonth : this.messages.prevMonth} class=${safeClassMap(s.chevron)} compact data-direction=${e ?? nothing} .disabled=${a} .icon=${t2 ? M2.chevronRight : M2.chevronLeft} icon-flip-rtl @click=${t2 ? this.nextMonthClick : this.prevMonthClick} @keydown=${t2 ? this.nextMonthKeydown : this.prevMonthKeydown} role=button .scale=${this.scale === "l" ? "l" : "m"} .text=${t2 ? this.messages.nextMonth : this.messages.prevMonth} ${ref((n) => t2 ? this.nextMonthAction = n : this.prevMonthAction = n)}></calcite-action>`;
  }
};
_H.properties = { nextMonthDate: 16, prevMonthDate: 16, activeDate: 0, headingLevel: 9, localeData: 0, max: 0, messages: 0, min: 0, monthStyle: 1, position: 1, scale: 3, selectedDate: 0 };
_H.styles = N2;
var H = _H;
...
@esri/calcite-components/dist/components/calcite-date-picker-month-header/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-PFVP6Z5A.js.map
