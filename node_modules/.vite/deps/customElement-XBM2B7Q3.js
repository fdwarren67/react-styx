...
var _R = class _R extends LitElement {
  constructor() {
    super(...arguments), this.actionBars = [], this.contentHeightMax = null, this.contentHeightMin = null, this.contentWidthMax = null, this.contentWidthMin = null, this.initialClientX = null, this.initialClientY = null, this.initialContentHeight = null, this.initialContentWidth = null, this.separatorPointerDown = (e) => {
      ...
      e.preventDefault();
      const { el: t, layout: i, initialContentWidth: n, initialContentHeight: m2, position: a, initialClientX: w, initialClientY: d } = this, l = i === "horizontal" ? e.clientY - d : e.clientX - w, s2 = i === "vertical" && gt(t) === "rtl" ? -1 : 1, x = a === "end" ? -s2 * l : s2 * l;
      i === "horizontal" ? this.setContentHeight(m2 + x) : this.setContentWidth(n + x);
    }, this.separatorPointerUp = (e) => {
      It(e) && (this.calciteInternalShellPanelResizeEnd.emit(), e.preventDefault(), window.removeEventListener("pointerup", this.separatorPointerUp), window.removeEventListener("pointermove", this.separatorPointerMove));
    }, this.step = 1, this.stepMultiplier = 10, this.contentHeight = null, this.contentWidth = null, this.hasHeader = false, this.collapsed = false, this.displayMode = "dock", this.layout = "vertical", this.messages = s(), this.position = "start", this.resizable = false, this.widthScale = "m", this.calciteInternalShellPanelResizeEnd = createEvent({ cancelable: false }), this.calciteInternalShellPanelResizeStart = createEvent({ cancelable: false });
  }
  // #endregion
  // #region Lifecycle
  willUpdate(e) {
  ...
  setContentWidth(e) {
    ...
    this.contentWidth = typeof t == "number" && typeof i == "number" ? m(n, i, t) : n;
  }
  updateAriaValues() {
  ...
  setContentHeight(e) {
    ...
    this.contentHeight = typeof t == "number" && typeof i == "number" ? m(n, i, t) : n;
  }
  updateWidths(e) {
  ...
  getKeyAdjustedSize(e) {
    const { key: t } = e, { el: i, step: n, stepMultiplier: m2, layout: a, contentWidthMin: w, contentWidthMax: d, initialContentWidth: l, initialContentHeight: s2, contentHeightMin: x, contentHeightMax: h, position: o } = this, c = n * m2;
    [
      "ArrowUp",
      "ArrowDown",
      "ArrowLeft",
    ...
    const k = gt(i), r2 = ["ArrowLeft", "ArrowRight"], p = ["ArrowDown", "ArrowUp"], f = k === "rtl" && r2.includes(t) ? -1 : 1;
    if (a === "horizontal" ? o === "end" ? t === p[1] || t === r2[0] : t === p[0] || t === r2[1] : t === p[1] || (o === "end" ? t === r2[0] : t === r2[1])) {
      const v = e.shiftKey ? c : n;
      return a === "horizontal" ? s2 + f * v : l + f * v;
    }
    if (a === "horizontal" ? o === "end" ? t === p[0] || t === r2[0] : t === p[1] || t === r2[1] : t === p[0] || (o === "end" ? t === r2[1] : t === r2[0])) {
      const v = e.shiftKey ? c : n;
      return a === "horizontal" ? s2 - f * v : l - f * v;
    }
    return t === "Home" && a === "horizontal" && typeof x == "number" ? x : t === "Home" && a === "vertical" && typeof w == "number" ? w : t === "End" && a === "horizontal" && typeof h == "number" ? h : t === "End" && a === "vertical" && typeof d == "number" ? d : t === "PageDown" ? a === "horizontal" ? s2 - c : l - c : t === "PageUp" ? a === "horizontal" ? s2 + c : l + c : null;
  }
  ...
  connectSeparator(e) {
    this.disconnectSeparator(), this.separatorEl = e, e == null ? void 0 : e.addEventListener("pointerdown", this.separatorPointerDown);
  }
  disconnectSeparator() {
    var _a;
  ...
  handleActionBarSlotChange(e) {
    const t = ut(e).filter((i) => i == null ? void 0 : i.matches("calcite-action-bar"));
  ...
  // #region Rendering
  renderHeader() {
    return keyed("header", html`<div class=${safeClassMap(u.contentHeader)} .hidden=${!this.hasHeader}><slot name=${$.header} @slotchange=${this.handleHeaderSlotChange}></slot></div>`);
  }
  render() {
    const { collapsed: e, position: t, initialContentWidth: i, initialContentHeight: n, contentWidth: m2, contentWidthMax: a, contentWidthMin: w, contentHeight: d, contentHeightMax: l, contentHeightMin: s2, resizable: x, layout: h, displayMode: o } = this, c = gt(this.el), C = o !== "float-content" && o !== "float" && x, k = C ? h === "horizontal" ? d ? { height: `${d}px` } : null : m2 ? { width: `${m2}px` } : null : null, r2 = !e && C ? keyed("separator", html`<div .ariaLabel=${this.messages.resize} .ariaOrientation=${h === "horizontal" ? "vertical" : "horizontal"} .ariaValueMax=${h == "horizontal" ? l : a} .ariaValueMin=${h == "horizontal" ? s2 : w} .ariaValueNow=${h == "horizontal" ? d ?? n : m2 ?? i} class=${safeClassMap(u.separator)} @keydown=${this.separatorKeyDown} role=separator tabindex=0 touch-action=none ${ref(this.connectSeparator)}></div>`) : null, p = () => h === "horizontal" ? t === "start" ? D.calciteAnimateInDown : D.calciteAnimateInUp : c === "ltr" && t === "end" || c === "rtl" && t === "start" ? D.calciteAnimateInLeft : D.calciteAnimateInRight, f = keyed("content", html`<div class=${safeClassMap({
      [D.rtl]: c === "rtl",
      [u.content]: true,
      [u.contentOverlay]: o === "overlay",
      [u.floatContent]: o === "float-content" || o === "float",
      [D.calciteAnimate]: o === "overlay",
      [p()]: o === "overlay",
      [r("width", this.width, this.widthScale)]: !!(this.width || this.widthScale),
      [r("height", this.height, this.heightScale)]: !!(this.height || this.heightScale)
    })} .hidden=${e} style=${safeStyleMap(k)} ${ref(this.storeContentEl)}>${this.renderHeader()}<div class=${safeClassMap(u.contentBody)}><slot></slot></div>${r2}</div>`), H = [keyed("action-bar", html`<slot name=${$.actionBar} @slotchange=${this.handleActionBarSlotChange}></slot>`), f];
    return t === "end" && H.reverse(), html`<div class=${safeClassMap({ [u.container]: true, [u.floatAll]: o === "float-all" })}>${H}</div>`;
  }
};
_R.properties = { contentHeight: 16, contentWidth: 16, hasHeader: 16, collapsed: 7, displayMode: 3, heightScale: 3, layout: 3, messageOverrides: 0, position: 3, resizable: 7, height: 3, widthScale: 3, width: 3 };
_R.styles = _;
var R = _R;
S("calcite-shell-panel", R);
export {
  R as ShellPanel
};
...
@esri/calcite-components/dist/components/calcite-shell-panel/customElement.js:
  ...
  v3.1.0 *)
*/
//# sourceMappingURL=customElement-XBM2B7Q3.js.map
