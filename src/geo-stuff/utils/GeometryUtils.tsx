...
export class GeometryUtils {
  public static degrees(pt1: Point, pt2: Point): number {
    return GeometryUtils.convertPathToDegrees([[pt1.x, pt1.y], [pt2.x, pt2.y]]);
  }

  public static convertPathToDegrees(path: number[][]): number {
  ...
  public static azimuthToRadians(azimuth: number) {
    let a2 = 90 - azimuth ;
    while (a2 < -180) {
      a2 += 360;
    }
  ...
  public static radians(pt1: Point, pt2: Point): number {
    return GeometryUtils.convertPathToRadians([[pt1.x, pt1.y], [pt2.x, pt2.y]]);
  }

  public static convertPathToRadians(path: number[][]): number {
    const dy = path[1][1] - path[0][1];
    const dx = path[1][0] - path[0][0];
  ...
  public static radians2(a: number[], b: number[]): number {
  ...
  public static rotate(geometry: Geometry, point: Point, degrees: number): GeometryUnion {
  ...
  public static pointsToPolygon(points: Point[]): Polygon {
    return new Polygon({
  ...
  public static offsetByAzimuth(orig: Point, azimuth: number, distance: number): Point {
  ...
  public static offsetByRadians(orig: Point, radians: number, distance: number): Point {
    const offsetX = Math.cos(radians) * distance;
    const offsetY = Math.sin(radians) * distance;
